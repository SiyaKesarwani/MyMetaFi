...
contract CollectionNFTCrowdfund is CollectiveCrowdfund {
    ...
    /// @notice This is the host of this crowdfund instance
    address public host;

    error OnlyCollectiveHost();

    /// @notice Set the `Globals` contract address.
    constructor(IGlobals globals) CollectiveCrowdfund(globals) {}

    function initialize(
        CollectionNFTCrowdfundOptions memory opts
    ) 
        external 
        payable 
        onlyConstructor
    ...
    /// @notice Execute arbitrary calldata to perform a buy, creating a collective
